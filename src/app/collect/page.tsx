...
async function getWasteCollectionTasks(limit: number = 20) {
  try {
...
async function saveReward(userId: number, amount: number) {
  try {
  
  const response = await axios.post('/api/savereward', { userId, amount });

  const reward = await response.data;
  if (response.status==200) { 
    console.log('Reward saved successfully:', reward);
  } else {
    console.error('Failed to save reward:', reward.error);
  }
} catch (error) {
...
export default function CollectPage() {
  ...
  async function saveCollectedWaste(reportId: number, collectorId: number, verificationResult: any) {
    try {
      const response = await axios.post('/api/savecollectedwaste',{ reportId, collectorId, verificationResult });

      const collectedWaste = await response.data;
      if (response.status==200) {
        console.log('Collected waste saved successfully:', collectedWaste);
      } else {
        console.error('Failed to save collected waste:', collectedWaste.error);
      }
    } catch (error) {
  ...
  async function updateTaskStatus(reportId: number, newStatus: string, collectorId?: number) {
    try {
      const response = await fetch('/api/updatetaskstatus', {
        ...
        body: JSON.stringify({ reportId, newStatus, collectorId }),
      });

      const updatedReport = await response.json();
      if (response.ok) {
        console.log('Task status updated successfully:', updatedReport);
      } else {
        console.error('Failed to update task status:', updatedReport.error);
      }
    } catch (error) {
